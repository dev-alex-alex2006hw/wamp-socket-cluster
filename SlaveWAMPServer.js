...
class SlaveWAMPServer extends WAMPServer {
	...
	constructor(
		...
		this.worker.on('masterMessage', (payload, cb) => {
			if (this.onRPC && payload.rpc) {
				this.onRPC(payload.peer, payload.procedure, payload.data, cb);
			}
			else if (this.onEvent && response.emit) {
				this.onEvent(payload.peer, payload.procedure, payload.data);
			}
